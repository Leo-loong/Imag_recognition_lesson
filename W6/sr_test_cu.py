...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 构建 D_lr 图像的文件路径
        # d_lr_path = os.path.join(self.d_lr_folder, self.file_names[idx].replace('rgb','depth'))
        # d_hr_path = os.path.join(self.d_hr_folder, self.file_names[idx].replace('rgb','depth').replace('.png','.bmp'))
...
def train_one_epoch(
    ...
    for i, (rgb_image, d_lr_image, d_hr_image) in enumerate(train_dataloader):
        ...
        optimizer.zero_grad()
        # 前向传播计算输出
        outputs = model(d_lr_image, rgb_image)
...
def test_epoch(args, epoch, test_dataloader, model, logger_val, criterion_mse, tb_logger):
    ...
    with torch.no_grad():
        for rgb_image, d_lr_image, d_hr_image in tqdm(test_dataloader):
            ...
            # 前向传播计算输出
            outputs = model(d_lr_image,rgb_image)
            print(outputs.max(),outputs.min(),d_hr_image.max(),d_hr_image.min(),d_lr_image.max(),rgb_image.max())
            # 计算均方误差损失
            mse_loss = criterion_mse(outputs, d_hr_image)
            # 计算总损失
            loss = mse_loss * 255 ** 2
